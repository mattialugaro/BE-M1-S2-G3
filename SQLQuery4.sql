...
-- SELECT CustomerID, SUM(Freight) AS TotaleSpeseTrasportoEffettuate FROM Orders GROUP BY CustomerID ORDER BY TotaleSpeseTrasportoEffettuate
...
-- SELECT OrderID, ProductID, UnitPrice, Quantity, SUM(UnitPrice * Quantity) AS PrezzoUnitaXQuantita FROM OrderDetails GROUP BY OrderID, ProductID, UnitPrice, Quantity
